...
const ExpandablePanel = ({children, heading, position="right"}) => {
  ...
  return (
    <div className={clsx('bg-transparent h-fit fixed z-50 top-20 w-fit p-6 font-iceland text-black flex justify-end items-start gap-2', position === "right" ? "right-0" : "left-0")}>
        {position === "right" && <Button variant='secondary' addClass='rounded-l-lg' size='sm' onClick={() => setIsExpanded(!isExpanded)}><FontAwesomeIcon icon={faGears}/></Button>}
        {isExpanded &&
        ...
        {position === "left" && <Button variant='secondary' addClass='rounded-r-lg' size='sm' onClick={() => setIsExpanded(!isExpanded)}><FontAwesomeIcon icon={faLightbulb}/></Button>}
    </div>
  )
}
